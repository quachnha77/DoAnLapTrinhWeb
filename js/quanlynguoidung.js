...
function loadData(currentPage, itemsPerPage) {
    if (typeof(Storage) !== "undefined") {
        var data = localStorage.getItem("accounts"); // Replace 'accounts' with your actual localStorage key
        if (data !== null) {
            try {
                var users = JSON.parse(data);
                displayUsers(users, currentPage, itemsPerPage);
...
function displayUsers(users, currentPage, itemsPerPage) {
    ...
    paginatedUsers.forEach(function(user, i) {
        var userElement = document.createElement('ul');
        userElement.innerHTML = 
            `<li class="id"><p>${i+1 || 'N/A'}</p></li>
             <li class="hoten"><p>${user.fullname || 'N/A'}</p></li>
             <li class="email"><p>${user.email || 'N/A'}</p></li>
             <li class="quyen"><p>${user.role || 'N/A'}</p></li>
             <li class="tacvu">
                    <div id="edit">
        ...
        editSVG.onclick = function() {
            // Lấy thông tin người dùng từ mảng "users" dựa trên chỉ mục "i"
            const currentUser = users[i];
            console.log(currentUser);

            // Set the value of the input fields
            document.getElementById("popupUserFullname").value = currentUser.fullname || '';
            document.getElementById("popupUserEmail").value = currentUser.email || '';

            // Set the selected option in the select element
            var roleSelect = document.getElementById("roleSelect"); // Make sure this is the correct ID of your <select> element
            for (var j = 0; j < roleSelect.options.length; j++) {
                if (roleSelect.options[j].value == currentUser.role) {
            ...
            saveButton.addEventListener('click', function() {
                var updatedFullname = document.getElementById('popupUserFullname').value;
                var updatedEmail = document.getElementById('popupUserEmail').value;
                var updatedRole = document.getElementById('roleSelect').value; // Ensure your select element has an ID
        ...
        removeButton.addEventListener('click', function() {
            ...
            // Tìm và xóa tài khoản dựa trên ID
...
timkiem.addEventListener('click', function() {
    ...
    users.forEach(function(user, i) {
        ...
        switch(searchType) {
            case 'Họ tên':
                match = fuzzySearch(input, user.fullname);
                break;
            case 'Email':
                match = fuzzySearch(input, user.email);
                break;
            case 'Quyền':
                match = fuzzySearch(input, user.role);
                break;
        }
